/** @file intellisenseDemo-solution.cpp
...
vector<vector<string>> searchSugesstion(vector <string> repository, string customerQuery)
{
	...
	{
		//cout << customerQuery.substr(0, i) << endl;
		customerQueries.push_back(customerQuery.substr(0, i));
	}

	vector<string> repositorySorted = repository;
	std::sort(repositorySorted.begin(), repositorySorted.end());
	//for (auto a : repositorySorted)
	//{
	//	cout << a << " ";
	//}
	//cout << endl;
	...
	for (auto q : customerQueries)
	{
		int count = 0;
		vector <string> resultTemp;
		bool insert = false;
		for (auto r : repositorySorted)
		{
			if (r.substr(0, (q.size())) == q && count < 3 )
			{			
				count++;
				//cout << r << " ";
				transform(r.begin(), r.end(), r.begin(), ::tolower);
				resultTemp.push_back(r);
				insert = true;
